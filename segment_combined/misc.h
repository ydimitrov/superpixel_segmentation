...
cv::Mat construct_hist (const cv::Mat &src_img) {
    ...
    calcHist(&hsv_base, 1, channels, cv::Mat(), hist, 2, histSize, ranges, true, false);
...
double euclidean_distance(std::vector<int> sample, std::vector<int> calculated_average) {
    return sqrt(pow((sample[0] - calculated_average[0]), 2) + pow((sample[1] - calculated_average[1]), 2) + pow((sample[2] - calculated_average[2]), 2));
}
