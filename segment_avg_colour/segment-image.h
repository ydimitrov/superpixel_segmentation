...
image<rgb> *segment_image(image<rgb> *im, float sigma, float c, int min_size,
  ...
  *num_ccs = u->num_sets();
  ...
  std::vector<int> sample_average = {255, 255, 27};     //21.ppm
  for(auto itr = final_components.begin(); itr != final_components.end(); itr++) {
    int component = *itr;
    std::vector<std::vector<int>> children = u->get_children(component, width, height);
    ...
    if(allowed_range_euclidean(sample_average, avg_colour, threshold)){
