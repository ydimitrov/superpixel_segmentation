...
image<rgb> *segment_image(image<rgb> *im, float sigma, float c, int min_size,
  ...
  *num_ccs = u->num_sets();
  ...
  cv::Mat sample_img_hist = construct_hist_hsv(convert_sample_to_nonzero_Mat(sample_img));
  ...
  for(auto itr = final_components.begin(); itr != final_components.end(); itr++) {
    ...
    cv::Mat component_hist = construct_hist_hsv(component_Mat);
    // if(component == 450966){
    //   segmentation_components.push_back(component);
    //   std::cout << "correlation thing = " << compareHist(sample_img_hist, component_hist, CV_COMP_CORREL) << std::endl;
    //   write_csv("component_450966_histogram.py", component_hist);
    // }
